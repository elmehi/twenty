...
- Use **Replace** in the settings after rotating your key. The app re-registers live sync with the new key. If the replacement key cannot delete the old endpoint, remove that endpoint in Granola's webhook settings.
- Granola drops events while an endpoint is paused, and the app never re-enables an endpoint on its own. The daily catch-up still imports notes updated in the last two days; use **Resume** in the settings to receive events again, and a larger manual import after a longer outage.
